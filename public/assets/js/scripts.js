...
function buscar_alumno(busca,arr){
	if(arr){
		console.log(arr);
	}
	
	console.log(busca,arr);
	var drop="";
	$.ajax({
		type: "GET",
		...
		success: function(respuesta){
			respuesta.forEach(function (alumno){
				drop += "<div  onclick='select_alumno(`"+ alumno.id + "`,`1`)'> "+ alumno.nombre+" "+ alumno.apellido1+" "+ alumno.apellido2+"</div>";
			});
...
function select_alumno(id,arr){
	...
	$.ajax({
		...
		success: function(respuesta){
			if(arr=='1'){
					var select = document.getElementById('implicados');
				    var opt = document.createElement('option');
					opt.value =  respuesta.id;
					opt.innerHTML =  respuesta.nombre + " " +  respuesta.apellido1 + " " +respuesta.apellido2;
					opt.selected=true;
